...
const API_BASE = 'https://bot.gowhats.in/api';
                ...
                {filteredTemplates.map((t) => (
                  ...
                  >
                    <div className="flex flex-col flex-1 min-w-0 mr-2 sm:mr-3">
                      <span className={`text-xs sm:text-sm font-semibold truncate ${value === t.name ? 'text-green-700' : 'text-gray-800'}`}>
                        {t.name}
                      </span>
...
const BroadcastReportModal = ({ broadcastId, onClose }) => {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const res = await fetch(`${API_BASE}/broadcasts/${broadcastId}/report`, {
          headers: authHeaders()
        });
  ...
  // FIX: useMemo to avoid recalculating on every render
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-2xl w-full sm:max-w-5xl h-[95vh] sm:h-[85vh] flex flex-col overflow-hidden">
        <div className="px-4 sm:px-6 py-4 sm:py-5 border-b-2 border-gray-100 flex justify-between items-center bg-gradient-to-r from-green-50 to-white">
          <div className="flex-1 min-w-0">
            ...
            {!error && !loading && <p className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1 font-medium truncate">{data?.broadcast?.name} • {filteredMessages.length} Messages</p>}
          </div>
              ...
              {loading ? (
                <div className="flex items-center justify-center h-full">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 font-medium text-sm sm:text-base">Loading report...</p>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <table className="hidden sm:table w-full text-left text-sm">
                    <thead className="bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700 font-semibold border-b-2 border-gray-200 sticky top-0">
                      <tr>
                        <th className="px-6 py-4 text-sm uppercase tracking-wide">Recipient</th>
                        <th className="px-6 py-4 text-sm uppercase tracking-wide">Status</th>
                        <th className="px-6 py-4 text-sm uppercase tracking-wide">Timestamp</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100 bg-white">
                      {filteredMessages.length === 0 ? (
                        <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-400">No messages found</td></tr>
                      ) : filteredMessages.map((msg) => (
...
// --- MOBILE BROADCAST CARD COMPONENT ---
    ...
    <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mt-0.5 font-medium truncate">{broadcast.templateName}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 bg-gray-50 rounded-lg p-3">
        <div className="text-center">
          <div className="font-bold text-gray-900 text-lg">{broadcast.sentCount ?? 0}</div>
          <div className="text-[10px] text-gray-500 font-medium uppercase">Sent</div>
        </div>
        <div className="text-center border-x border-gray-200">
          <div className="font-bold text-green-600 text-lg">{broadcast.deliveredCount ?? 0}</div>
          <div className="text-[10px] text-gray-500 font-medium uppercase">Delivered</div>
        </div>
        <div className="text-center">
          <div className="font-bold text-green-700 text-lg">{broadcast.readCount ?? 0}</div>
...
export default function BroadcastMessage() {
  ...
  const [broadcastForm, setBroadcastForm] = useState(DEFAULT_FORM);
  ...
  // FIX: useMemo for derived template state
  const selectedTemplateDetails = useMemo(
    () => templates.find(t => t.name === broadcastForm.templateName),
    [templates, broadcastForm.templateName]
  );
  const carouselComponent = useMemo(
    () => selectedTemplateDetails?.components?.find(c => c.type === 'CAROUSEL'),
    [selectedTemplateDetails]
  );
  const isCarousel = !!carouselComponent;
  const requiresMedia = useMemo(
    () => !isCarousel && selectedTemplateDetails?.components?.some(c => c.type === 'HEADER' && ['IMAGE', 'VIDEO', 'DOCUMENT'].includes(c.format)),
    [isCarousel, selectedTemplateDetails]
  );
  const mediaTypeRequired = useMemo(
    () => selectedTemplateDetails?.components?.find(c => c.type === 'HEADER')?.format,
    [selectedTemplateDetails]
  );

  // FIX: memoized filtered contacts
  ...
  useEffect(() => {
    if (subscriptionLoading || !hasProAccess) return;
    fetchBroadcasts();
    fetchTemplates();
    fetchContacts();
  ...
  const fetchContacts = async () => {
    try {
      const res = await fetch(`${API_BASE}/contacts?limit=1000`, { headers: authHeaders() });
      const data = await res.json();
      if (Array.isArray(data?.contacts)) setAvailableContacts(data.contacts);
      else if (Array.isArray(data)) setAvailableContacts(data);
      else { console.error('Unexpected contacts format:', data); setAvailableContacts([]); }
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${API_BASE}/broadcasts/${deleteConfirm.id}`, {
        method: 'DELETE',
        headers: authHeaders()
      });
      const data = await res.json();
      if (data.success) { toast.success('Campaign deleted'); fetchBroadcasts(); }
  ...
  // FIX: Extracted shared step 1 validation to avoid duplication
  const validateStep1Fields = () => {
    const newErrors = {};
    let isValid = true;

    if (!broadcastForm.name.trim()) { newErrors.name = 'Please enter a campaign name'; isValid = false; }
    if (!broadcastForm.audience) { newErrors.audience = 'Please select a target audience'; isValid = false; }
    if (broadcastForm.audience === 'custom' && selectedContactPhones.length === 0) {
      newErrors.customContacts = 'Please select at least one contact'; isValid = false;
    }
    if (broadcastForm.audience === 'import' && !importFile && selectedContactPhones.length === 0) {
      newErrors.importFile = 'Please upload and validate a CSV file'; isValid = false;
    }
  ...
  const getAudiencePreview = async () => {
    ...
    // Local audience types don't need API call
    if (broadcastForm.audience === 'custom' || broadcastForm.audience === 'import') {
      ...
      setAutoSelectedRecipients(selectedContactPhones);
      setEstimatedReach(selectedContactPhones.length);
      setStep(2);
      return;
    }
    ...
    try {
      const payload = {
        ...
        filters: { orderFilters, bookingFilters, orderStatus: broadcastForm.orderStatusFilter }
      };
      const res = await fetch(`${API_BASE}/broadcasts/audience/preview`, {
        method: 'POST',
      ...
      const data = await res.json();
      if (data.success) {
        const finalRecipients = data.phoneNumbers || [];
  ...
  const handleSubmitBroadcast = async () => {
    ...
    if (!broadcastForm.templateName) { newErrors.templateName = 'Please select a template'; isValid = false; }
    if (requiresMedia && !broadcastForm.mediaFile) { newErrors.mediaFile = 'Please upload header media'; isValid = false; }

    if (isCarousel && carouselComponent) {
      carouselComponent.cards.forEach((_, index) => {
        if (!carouselFiles[index]) { newErrors[`carousel_${index}`] = 'Image required'; isValid = false; }
      });
    }

    if (broadcastForm.scheduled && !broadcastForm.scheduledDateTime) {
      newErrors.scheduledDateTime = 'Please select schedule date and time'; isValid = false;
    }

    // FIX: scheduled date must be in the future
    if (broadcastForm.scheduled && broadcastForm.scheduledDateTime) {
      const scheduledTime = new Date(broadcastForm.scheduledDateTime).getTime();
    ...
    } else if (broadcastForm.audience === 'all') {
      finalRecipients = availableContacts.map(c => c.phone_number);
    } else {
      finalRecipients = autoSelectedRecipients;
    }

    if (finalRecipients.length === 0) { toast.error('No recipients selected'); return; }
    ...
    try {
      ...
      if (isCarousel && carouselComponent) {
        carouselComponent.cards.forEach((_, i) => {
  ...
  // FIX: Extracted reset logic to avoid repetition
  const resetModal = () => {
    setStep(1);
    setBroadcastForm(DEFAULT_FORM);
    setCarouselFiles({});
    setSelectedContactPhones([]);
    setAutoSelectedRecipients([]);
    setImportFile(null);
    setImportStats(null);
    setEstimatedReach(0);
    setErrors({});
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/broadcasts/import/validate`, {
  ...
  const finalizeImport = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/broadcasts/import/contacts`, {
        method: 'POST', headers: authHeaders(), body: formData
      });
      const data = await res.json();
      if (data.success) {
        setSelectedContactPhones(data.phoneNumbers);
        setEstimatedReach(data.phoneNumbers.length);
        setBroadcastForm(prev => ({ ...prev, audience: 'import' }));
        toast.success(`Imported ${data.phoneNumbers.length} contacts`);
      } else {
  ...
  const selectAllContacts = () => {
    clearError('customContacts');
    if (selectedContactPhones.length === availableContacts.length) {
      setSelectedContactPhones([]);
    } else {
      setSelectedContactPhones(availableContacts.map(c => c.phone_number));
    }
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-gray-50 to-green-50/20 min-h-screen font-sans">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl border-2 border-gray-100 shadow-sm mb-4 sm:mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          onClick={() => { setShowCreateModal(true); setErrors({}); resetModal(); }}
      ...
      <div className="hidden lg:block bg-white rounded-xl shadow-sm border-2 border-gray-100 overflow-hidden">
        ...
        {broadcasts.length === 0 ? (
          <div className="p-12 text-center">
            <div className="text-gray-300 mb-3"><BarChart2 size={48} className="mx-auto" /></div>
          ...
          <div key={b._id} className="grid grid-cols-12 gap-4 px-6 py-5 items-center hover:bg-green-50/30 border-b border-gray-100 last:border-0 transition-colors">
            ...
            <div className="col-span-2">
              <span className="text-xs font-bold bg-green-50 text-green-700 px-3 py-1.5 rounded-lg border border-green-200 capitalize">{b.audienceType}</span>
            </div>
            <div className="col-span-3 flex justify-center gap-6 text-sm">
              <div className="text-center"><div className="font-bold text-gray-900 text-lg">{b.sentCount ?? 0}</div><div className="text-xs text-gray-500 font-medium">Sent</div></div>
              <div className="text-center"><div className="font-bold text-green-600 text-lg">{b.deliveredCount ?? 0}</div><div className="text-xs text-gray-500 font-medium">Delivered</div></div>
              <div className="text-center"><div className="font-bold text-green-700 text-lg">{b.readCount ?? 0}</div><div className="text-xs text-gray-500 font-medium">Read</div></div>
            </div>
            <div className="col-span-2 text-center">
              <span className={`text-xs font-bold px-3 py-1.5 rounded-lg border uppercase ${
                b.status === 'completed' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-50 text-gray-600 border-gray-200'
              }`}>{b.status}</span>
            </div>
            <div className="col-span-1 text-right flex justify-end gap-2">
              <button onClick={() => handleViewReport(b._id)} className="p-2.5 border-2 border-gray-200 rounded-lg hover:bg-green-50 hover:border-green-300 transition-all text-gray-600 hover:text-green-600"><BarChart2 size={18}/></button>
              <button onClick={() => handleDeleteBroadcast(b._id, b.name)} className="p-2.5 border-2 border-gray-200 rounded-lg hover:bg-red-50 hover:border-red-300 transition-all text-gray-600 hover:text-red-600"><Trash2 size={18}/></button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="lg:hidden space-y-3">
        {broadcasts.length === 0 ? (
          <div className="bg-white rounded-xl p-8 text-center border-2 border-gray-100">
            <div className="text-gray-300 mb-3"><BarChart2 size={40} className="mx-auto" /></div>
            <p className="text-gray-500 font-semibold">No campaigns yet</p>
            <p className="text-gray-400 text-sm mt-1">Create your first broadcast campaign</p>
          </div>
        ) : broadcasts.map((b, i) => (
          <MobileBroadcastCard key={b._id} broadcast={b} index={i} onViewReport={handleViewReport} onDelete={handleDeleteBroadcast} />
        ))}
      </div>

      {showReportModal && (
        <BroadcastReportModal broadcastId={selectedBroadcastId} onClose={() => { setShowReportModal(false); setSelectedBroadcastId(null); }} />
      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="p-6">
              ...
              <div className="flex gap-3">
                <button onClick={() => setDeleteConfirm({ show: false, id: null, name: '' })} className="flex-1 py-2.5 border-2 border-gray-200 rounded-xl font-bold text-gray-600 hover:bg-gray-50 transition-all text-sm">Cancel</button>
                <button onClick={confirmDelete} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold transition-all text-sm shadow-lg shadow-red-200">Yes, Delete</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-3xl overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[90vh]">
            ...
            <div className="px-4 sm:px-8 py-4 sm:py-6 border-b-2 border-gray-100 bg-gradient-to-r from-green-50 to-white flex justify-between items-center">
              ...
              <button onClick={() => { setShowCreateModal(false); resetModal(); }} className="p-2 hover:bg-white rounded-lg border border-transparent hover:border-gray-200 transition-all text-gray-500 hover:text-gray-700">
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => { setBroadcastForm(prev => ({...prev, name: e.target.value})); clearError('name'); }}
                      className={`w-full border-2 p-3 sm:p-3.5 rounded-lg outline-none transition-all font-medium text-sm sm:text-base ${
                        errors.name ? 'border-red-500 focus:ring-2 focus:ring-red-100 focus:border-red-500 bg-red-50/10' : 'border-gray-200 focus:ring-2 focus:ring-green-100 focus:border-green-500'
                      }`}
                      placeholder="e.g. Diwali Sale Campaign"
                    />
                    {errors.name && <p className="text-red-500 text-xs font-bold mt-1.5 flex items-center gap-1"><AlertCircle className="w-3 h-3"/> {errors.name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-5 gap-2 sm:gap-4">
                      {[
                        ...
                        <button
                          key={type.id}
                          onClick={() => { setBroadcastForm(prev => ({...prev, audience: type.id})); clearError('audience'); }}
                          className={`p-3 sm:p-4 rounded-xl border-2 flex flex-col items-center gap-1.5 sm:gap-2 transition-all ${
                            broadcastForm.audience === type.id ? 'border-green-500 bg-green-50 text-green-700 shadow-sm' : 'border-gray-200 hover:border-green-300 hover:bg-green-50/30'
                          }`}
                        >
                    ...
                    {errors.audience && <p className="text-red-500 text-xs font-bold mt-2 flex items-center gap-1"><AlertCircle className="w-3 h-3"/> {errors.audience}</p>}
                  </div>
                    ...
                    <div className="bg-blue-50 p-4 sm:p-5 rounded-xl border-2 border-blue-200">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {['all', 'completed', 'pending'].map(status => (
                          <button key={status} onClick={() => setBroadcastForm(prev => ({...prev, orderStatusFilter: status}))}
                            className={`px-3 sm:px-4 py-2 rounded-lg text-xs font-bold capitalize transition-all ${broadcastForm.orderStatusFilter === status ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-blue-600 border border-blue-200 hover:bg-blue-100'}`}>{status}</button>
                        ))}
                      </div>
                      <p className="text-xs text-blue-700 mt-2">Sending to: <b>{broadcastForm.orderStatusFilter.toUpperCase()}</b> orders</p>
                    </div>
                  )}
                    ...
                    <div className="bg-purple-50 p-4 sm:p-5 rounded-xl border-2 border-purple-200">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {['all', 'active', 'used', 'pending_payment'].map(status => (
                          <button key={status} onClick={() => setBookingFilters({ status })}
                            className={`px-3 sm:px-4 py-2 rounded-lg text-xs font-bold capitalize transition-all ${bookingFilters.status === status ? 'bg-purple-600 text-white shadow-md' : 'bg-white text-purple-600 border border-purple-200 hover:bg-purple-100'}`}>{status.replace('_', ' ')}</button>
                        ))}
                      </div>
                      <p className="text-xs text-purple-700 mt-2">Sending to: <b>{bookingFilters.status.toUpperCase()}</b> bookings</p>
                    </div>
                  )}
                  ...
                  {broadcastForm.audience === 'custom' && (
                    <div className={`p-4 sm:p-5 rounded-xl border-2 ${errors.customContacts ? 'border-red-200 bg-red-50/10' : 'bg-green-50 border-green-100'}`}>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <button onClick={selectAllContacts} className={`text-xs font-bold bg-white px-2.5 sm:px-3 py-1.5 rounded-lg border ${errors.customContacts ? 'text-red-700 border-red-200' : 'text-green-700 border-green-200'}`}>
                      ...
                      <div className="max-h-48 sm:max-h-56 overflow-y-auto space-y-2">
                        {filteredContacts.map(c => (
                          <label key={c._id} className={`flex gap-3 p-2.5 sm:p-3 bg-white border-2 rounded-lg cursor-pointer transition-colors ${errors.customContacts ? 'border-red-100 hover:bg-red-50' : 'border-green-100 hover:bg-green-50/50'}`}>
                            <input type="checkbox" checked={selectedContactPhones.includes(c.phone_number)} onChange={() => toggleContactSelection(c.phone_number)} className="w-4 h-4 accent-green-600 flex-shrink-0 mt-0.5"/>
                      ...
                      <div className="mt-3 p-2.5 sm:p-3 bg-white rounded-lg border border-green-200 flex justify-between items-center">
                        <p className="text-xs font-bold text-green-700">Selected: {selectedContactPhones.length} contacts</p>
                      </div>
                      {errors.customContacts && <p className="text-red-500 text-xs font-bold mt-2 flex items-center gap-1"><AlertCircle className="w-3 h-3"/> {errors.customContacts}</p>}
                    </div>
                  )}
                  ...
                  {broadcastForm.audience === 'import' && (
                    <div className={`p-4 sm:p-5 rounded-xl border-2 ${errors.importFile ? 'border-red-200 bg-red-50/10' : 'bg-green-50 border-green-200'}`}>
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-3">
                        ...
                        <button onClick={downloadSampleCsv} className="text-xs text-green-700 font-bold hover:underline flex items-center gap-1">
                      ...
                      <input
                        ...
                        accept=".csv,.xlsx"
                        className={`block w-full text-sm text-gray-600 file:mr-3 sm:file:mr-4 file:py-2 sm:file:py-2.5 file:px-3 sm:file:px-4 file:rounded-lg file:border-0 file:text-xs sm:file:text-sm file:font-bold file:cursor-pointer border-2 rounded-lg bg-white ${errors.importFile ? 'border-red-200 file:bg-red-500 file:text-white' : 'border-green-200 file:bg-green-600 file:text-white hover:file:bg-green-700'}`}
                      />
                      {errors.importFile && <p className="text-red-500 text-xs font-bold mt-2 flex items-center gap-1"><AlertCircle className="w-3 h-3"/> {errors.importFile}</p>}
                      <div className="mt-2 text-[10px] text-gray-500">ℹ️ File must contain "Name" and "Phone" columns.</div>
                      {importStats && (
                        <>
                          ...
                          <button onClick={finalizeImport} className="mt-3 w-full bg-green-600 hover:bg-green-700 text-white px-4 py-2.5 rounded-lg text-sm font-bold transition-all">
                            Import Contacts
                          </button>
                        </>
                      )}
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div>
                    ...
                    <TemplateSelector
                      ...
                      onChange={(val) => {
                        setBroadcastForm(prev => ({...prev, templateName: val, mediaFile: null}));
                        setCarouselFiles({});
                        setErrors({});
                        ...
                        {carouselComponent.cards.map((card, index) => {
                          ...
                          return (
                            <div key={index} className={`p-3 sm:p-4 rounded-lg border-2 flex items-center justify-between gap-3 sm:gap-4 ${hasError ? 'bg-red-50 border-red-200' : 'bg-white border-green-100'}`}>
                              <div className="flex-1 min-w-0">
                                <label className={`text-[10px] sm:text-xs font-bold block mb-1.5 sm:mb-2 ${hasError ? 'text-red-700' : 'text-gray-700'}`}>Card {index + 1} ({format})</label>
                                <input type="file" accept={acceptType}
                                  className={`w-full text-[10px] sm:text-xs text-gray-600 file:mr-2 sm:file:mr-4 file:py-1.5 sm:file:py-2 file:px-2 sm:file:px-4 file:rounded-lg file:border-0 file:text-[10px] sm:file:text-xs file:font-bold file:cursor-pointer ${hasError ? 'file:bg-red-100 file:text-red-700' : 'file:bg-green-50 file:text-green-700 hover:file:bg-green-100'}`}
                                  onChange={(e) => { const file = e.target.files[0]; setCarouselFiles(prev => ({...prev, [index]: file})); clearError(`carousel_${index}`); }}
                                />
                                {hasError && <p className="text-red-500 text-[10px] font-bold mt-1">{hasError}</p>}
                              </div>
                              {carouselFiles[index] ? <CheckCircle className="text-green-500 w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" /> : <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex-shrink-0 ${hasError ? 'border-red-300' : 'border-gray-300'}`}></div>}
                            </div>
                          );
                        })}
                      </div>
                  ...
                  {requiresMedia && !isCarousel && (
                    <div className={`p-4 sm:p-5 rounded-xl border-2 ${errors.mediaFile ? 'border-red-200 bg-red-50/10' : 'bg-green-50 border-green-200'}`}>
                      ...
                      <input
                        type="file"
                        className={`w-full bg-white p-2.5 sm:p-3 rounded-lg border-2 text-sm file:mr-3 sm:file:mr-4 file:py-1.5 sm:file:py-2 file:px-3 sm:file:px-4 file:rounded-lg file:border-0 file:text-xs sm:file:text-sm file:font-bold file:cursor-pointer ${errors.mediaFile ? 'border-red-200 file:bg-red-600 file:text-white hover:file:bg-red-700' : 'border-green-200 file:bg-green-600 file:text-white hover:file:bg-green-700'}`}
                        accept={mediaTypeRequired === 'IMAGE' ? 'image/*' : 'video/*'}
                        onChange={e => { setBroadcastForm(prev => ({...prev, mediaFile: e.target.files[0]})); clearError('mediaFile'); }}
                      />
                      {errors.mediaFile && <p className="text-red-500 text-xs font-bold mt-2 flex items-center gap-1"><AlertCircle className="w-3 h-3"/> {errors.mediaFile}</p>}
                  ...
                  <div>
                    <label className="flex items-center gap-3 p-3 sm:p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:bg-green-50 hover:border-green-300 transition-all">
                      <input type="checkbox" checked={broadcastForm.scheduled}
                        onChange={e => { setBroadcastForm(prev => ({...prev, scheduled: e.target.checked})); clearError('scheduledDateTime'); }}
                    ...
                    {broadcastForm.scheduled && (
                      <div className={`mt-3 sm:mt-4 p-3 sm:p-4 rounded-xl border-2 ${errors.scheduledDateTime ? 'border-red-200 bg-red-50/10' : 'bg-green-50 border-green-200'}`}>
                        ...
                        <input
                          type="datetime-local"
                          // FIX: prevent past dates via min attribute
                          ...
                          onChange={e => { setBroadcastForm(prev => ({...prev, scheduledDateTime: e.target.value})); clearError('scheduledDateTime'); }}
                        />
                        {errors.scheduledDateTime && <p className="text-red-500 text-xs font-bold mt-2 flex items-center gap-1"><AlertCircle className="w-3 h-3"/> {errors.scheduledDateTime}</p>}
                      </div>
                    )}
                  </div>
            ...
            <div className="p-4 sm:p-6 border-t-2 border-gray-100 bg-gradient-to-r from-gray-50 to-white flex justify-between items-center gap-3">
              {step > 1 ? (
                <button onClick={() => setStep(step - 1)} disabled={isSending} className="text-gray-600 font-bold hover:text-gray-900 disabled:opacity-50 px-3 sm:px-4 py-2 hover:bg-gray-100 rounded-lg transition-all text-sm sm:text-base">← Back</button>
              ) : <div></div>}

              {step < 2 ? (
                <button onClick={getAudiencePreview} disabled={isSending} className="bg-green-600 text-white px-5 sm:px-8 py-2.5 sm:py-3 rounded-xl font-bold hover:bg-green-700 transition-all shadow-lg shadow-green-200 hover:shadow-xl text-sm sm:text-base disabled:bg-gray-400 disabled:cursor-not-allowed">
                  {isSending ? <div className="animate-spin rounded-full h-4 w-4 sm:h-5 sm:w-5 border-2 border-white border-t-transparent"></div> : 'Next Step →'}
                </button>
              ) : (
                <button onClick={handleSubmitBroadcast} disabled={isSending} className="bg-green-600 text-white px-5 sm:px-8 py-2.5 sm:py-3 rounded-xl font-bold hover:bg-green-700 transition-all flex items-center gap-2 disabled:bg-gray-400 disabled:cursor-not-allowed shadow-lg shadow-green-200 hover:shadow-xl text-sm sm:text-base">
                  {isSending ? <div className="animate-spin rounded-full h-4 w-4 sm:h-5 sm:w-5 border-2 border-white border-t-transparent"></div> : <Send className="w-4 h-4 sm:w-5 sm:h-5"/>}
