...
const ProductFormModal = ({ isOpen, onClose, product, onSave, isSaving }) => {
  ...
  const [expandedSections, setExpandedSections] = useState({
    basic: true, pricing: true, media: true, gallery: false,
  });

  // ── FIX: Use refs for uncontrolled inputs to prevent cursor jumping ──
  const formRef = useRef({});

  // ── FIX: Initialize form data only when modal opens or product changes ──
  ...
  useEffect(() => {
    if (isOpen) {
      const initial = isEditMode
        ...
        : {
            name: "", retailer_id: "", color: "", size: "", variant_group: "",
            description: "", price: "", inventory: "", condition: "new",
            availability: "in stock", image_url: "", additional_images: [], url: "",
          };
      setFormData(initial);
      // Sync ref with initial data
      formRef.current = { ...initial };
      setActiveTab("basic");
    }
  }, [isOpen, product?._id]); // ── FIX: depend on product._id not entire product object

  // ── FIX: Use a stable onChange that updates ref immediately (no re-render)
  // and only syncs to state for fields that affect UI display (image preview, counters, etc.)
  const handleInputChange = useCallback((e) => {
    const { name, value } = e.target;
    // Always update the ref (this is what gets submitted)
    formRef.current = { ...formRef.current, [name]: value };

    // Only trigger re-render for fields that have live UI side-effects
    const fieldsNeedingRerender = ["image_url", "description", "availability", "condition"];
    if (fieldsNeedingRerender.includes(name)) {
      setFormData(prev => ({ ...prev, [name]: value }));
    }
  }, []);

  const handleAddImage = () => {
    const currentImages = formRef.current.additional_images || [];
    if (currentImages.length >= 10) return;
    const newImages = [...currentImages, ""];
    formRef.current = { ...formRef.current, additional_images: newImages };
    setFormData(prev => ({ ...prev, additional_images: newImages }));
  };

  const handleAdditionalImageChange = (index, value) => {
    const newImages = [...(formRef.current.additional_images || [])];
  ...
  // ── FIX: Read from ref at submit time, not from stale state ──
  const handleSubmit = (e) => {
    e.preventDefault();
    const latest = { ...formRef.current };

    // ── FIX: Parse price cleanly to avoid floating point drift ──
    const rawPrice = parseFloat(latest.price);
    latest.price = Number.isFinite(rawPrice) ? Math.round(rawPrice * 100) / 100 : 0;

    const rawInventory = parseInt(latest.inventory, 10);
    latest.inventory = Number.isFinite(rawInventory) ? rawInventory : null;

    latest.additional_images = (latest.additional_images || []).filter(img => img.trim() !== "");
  ...
  const SectionHeader = ({ icon: Icon, title, subtitle, section, badge }) => (
    <button
      type="button"
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        {expandedSections[section] ? <ChevronUp className="h-4 w-4 text-gray-400" /> : <ChevronDown className="h-4 w-4 text-gray-400" />}
      </div>
    </button>
  );
  ...
  const inputClass = "w-full rounded-lg border border-gray-300 bg-white py-2.5 px-4 text-sm shadow-sm placeholder:text-gray-400 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500/20 transition-all";
          ...
          <div className="flex gap-1 mt-4 p-1 bg-gray-100 rounded-lg">
            {tabs.map(tab => (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-md text-xs sm:text-sm font-medium transition-all ${
                  activeTab === tab.id ? "bg-white text-green-700 shadow-sm" : "text-gray-600 hover:text-gray-900 hover:bg-gray-50"
                }`}
              >
      ...
      <form onSubmit={handleSubmit}>
        <div className="max-h-[55vh] overflow-y-auto">
          ...
          {activeTab === "basic" && (
            <div className="p-4 sm:p-6 space-y-4">
              <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                <SectionHeader icon={ShoppingBag} title="Product Details" subtitle="Name, SKU and description" section="basic" />
                {expandedSections.basic && (
                  <div className="p-4 space-y-4 bg-white">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <FormField label="Product Name" icon={Tag} required>
                        <div className="relative">
                          {/* ── FIX: defaultValue + uncontrolled for cursor stability ── */}
                          <input
                            name="name"
                            defaultValue={formData.name || ""}
                            key={`name-${product?._id || "new"}`}
                      ...
                      <FormField label="SKU / Product ID" icon={Hash} required hint={isEditMode ? "Read-only" : ""}>
                        <div className="relative">
                          <input
                            name="retailer_id"
                            defaultValue={formData.retailer_id || ""}
                            key={`sku-${product?._id || "new"}`}
                            onChange={handleInputChange}
                            placeholder="e.g., SKU-12345"
                            required
                            readOnly={isEditMode}
                            className={`${inputClass} ${isEditMode ? "bg-gray-50 text-gray-600 cursor-not-allowed" : ""}`}
                          />
                          {isEditMode && (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <FormField label="Color" icon={Tag}>
                        <input
                          name="color"
                          defaultValue={formData.color || ""}
                          key={`color-${product?._id || "new"}`}
                          onChange={handleInputChange}
                          placeholder="e.g., Maroon"
                          className={inputClass}
                        />
                      </FormField>
                      <FormField label="Size" icon={Box}>
                        <input
                          name="size"
                          defaultValue={formData.size || ""}
                          key={`size-${product?._id || "new"}`}
                          onChange={handleInputChange}
                          placeholder="e.g., Free Size"
                          className={inputClass}
                        />
                      </FormField>
                      <FormField label="Variant Group" icon={Layers}>
                        <input
                          name="variant_group"
                          defaultValue={formData.variant_group || ""}
                          key={`vg-${product?._id || "new"}`}
                          onChange={handleInputChange}
              ...
              <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                <SectionHeader icon={DollarSign} title="Pricing & Inventory" subtitle="Set price and stock levels" section="pricing" />
                {expandedSections.pricing && (
                  <div className="p-4 space-y-4 bg-white">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <FormField label="Price" icon={DollarSign} required>
                        <div className="relative">
                          ...
                          {/* ── FIX: type="text" with inputMode to avoid browser float adjustments ── */}
                          <input
                            name="price"
                            type="text"
                            inputMode="decimal"
                            defaultValue={formData.price || ""}
                            key={`price-${product?._id || "new"}`}
                            onChange={handleInputChange}
                            placeholder="0.00"
                            required
                      ...
                      <FormField label="Inventory Quantity" icon={Box}>
                        <input
                          name="inventory"
                          type="text"
                          inputMode="numeric"
                          defaultValue={formData.inventory ?? ""}
                          key={`inv-${product?._id || "new"}`}
                          onChange={handleInputChange}
                        ...
                        <div className="flex flex-wrap gap-2">
                          {[
                            ...
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                formRef.current = { ...formRef.current, condition: option.value };
                                setFormData(prev => ({ ...prev, condition: option.value }));
                              }}
                              title={option.fullLabel || option.label}
                              className={`flex-1 min-w-[70px] py-2.5 px-2 rounded-lg text-xs font-medium border-2 transition-all text-center whitespace-nowrap ${
                        ...
                        <div className="flex gap-2">
                          {[
                            ...
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                formRef.current = { ...formRef.current, availability: option.value };
                                setFormData(prev => ({ ...prev, availability: option.value }));
                              }}
                              className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 px-2 rounded-lg text-xs font-medium border-2 transition-all whitespace-nowrap ${
                    ...
                    <FormField label="Product Link (Optional)" icon={Link}>
                      <input
                        name="url"
                        type="url"
                        defaultValue={formData.url || ""}
                        key={`url-${product?._id || "new"}`}
                        onChange={handleInputChange}
          ...
          {activeTab === "media" && (
            <div className="p-4 sm:p-6 space-y-4">
              <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                <SectionHeader icon={ImageIcon} title="Main Product Image" subtitle="Primary display image" section="media" />
                        ...
                        <div className="relative group">
                          ...
                          {formData.image_url && (
                            <button
                              type="button"
                              onClick={() => {
                                formRef.current = { ...formRef.current, image_url: "" };
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="flex items-start gap-2 p-3 bg-blue-50 rounded-lg">
                          ...
                          <p className="text-xs text-blue-700">
                            Use high-resolution images (min 800×800px). Supported: JPG, PNG, WebP
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                <SectionHeader
                  ...
                  subtitle="Additional product images"
                  section="gallery"
                  badge={`${formData.additional_images?.length || 0}/10`}
                />
                  ...
                  <div className="p-4 bg-white space-y-4">
                    ...
                    <button
                      ...
                      className={`w-full py-3 px-4 rounded-lg border-2 border-dashed transition-all flex items-center justify-center gap-2 ${
                        (formData.additional_images?.length || 0) >= 10
                    ...
                    <div className="sm:w-3/5 p-5">
                      ...
                      <div className="flex items-baseline gap-2 mb-4">
                        <span className="text-3xl font-bold text-green-600">
                          ₹{formRef.current.price || formData.price || "0.00"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
                        <Box className="h-4 w-4" />
                        <span>{formRef.current.inventory || formData.inventory || 0} units in stock</span>
                      </div>
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              ...
              <Button type="submit" disabled={isSaving} className={`px-6 py-2.5 flex-1 sm:flex-none ${isSaving ? "opacity-80 cursor-not-allowed" : ""}`}>
                {isSaving ? (
                  ...
                  <><CheckCircle className="h-4 w-4" /> {isEditMode ? "Update Product" : "Create Product"}</>
                )}
              </Button>
            </div>
          </div>
    ...
    <div className="group flex flex-col overflow-hidden rounded-xl border bg-white shadow-sm ring-1 ring-black/5 transition hover:shadow-lg">
      <div className="relative">
        <div className="aspect-[4/3] w-full bg-gray-100 relative">
          <img src={safeImg} alt={item.name} className="h-full w-full object-cover"
          ...
          {hasExtraImages && (
            <div className="absolute bottom-2 right-2 bg-black/60 text-white text-[10px] px-1.5 py-0.5 rounded flex items-center gap-1 backdrop-blur-sm">
              <ImageIcon size={10} /><span>+{item.additional_images.length}</span>
            </div>
          )}
        </div>
      ...
      <div className="flex flex-1 flex-col p-3 sm:p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="relative">
            <button onClick={() => setMenuOpen(s => !s)} onBlur={() => setTimeout(() => setMenuOpen(false), 150)}
        ...
        {(item.color || item.size) && (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {item.color && <Badge tone="red">Color: {item.color}</Badge>}
            {item.size && <Badge tone="blue">Size: {item.size}</Badge>}
          </div>
        )}
        ...
        <div className="mt-2 sm:mt-3 flex items-center justify-between">
          <span className="text-[10px] sm:text-xs text-gray-400">
            {item.variant_group ? `Group: ${item.variant_group}` : `Condition: ${item.condition || "-"}`}
          </span>
        </div>
      </div>
    </div>
...
const DEFAULT_ALERT_CONFIG = {
  ...
  templateName: "low_stock_alertt", templateLanguage: "en",
  messageTemplate: "Low stock alert: {{productName}} ({{retailerId}}) is now at {{currentStock}} units.",
  ceoPhone: "", adminPhone: "",
};
...
const ViewInventory = () => {
  ...
  useEffect(() => { fetchInventoryData(); fetchAlertConfig(); }, []);

  const fetchInventoryData = async () => {
    setIsLoading(true);
    try {
      const token = localStorage.getItem("token");
      const response = await publicApi.get("/api/inventory", { headers: { Authorization: `Bearer ${token}` } });
      setInventoryData(response.data);
    } catch { showErrorModal("Failed to fetch inventory data."); }
  ...
  const fetchAlertConfig = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await publicApi.get("/api/inventory/alerts-config", { headers: { Authorization: `Bearer ${token}` } });
      ...
      setAlertConfig({
        ...
        ceoPhone: config.ceoPhone || "", adminPhone: config.adminPhone || "",
      });
    } catch { showErrorModal("Failed to load low stock alert settings."); }
    finally { setIsAlertConfigLoading(false); }
  };

  const handleAlertConfigChange = (key, value) => setAlertConfig(prev => ({ ...prev, [key]: value }));
  ...
  const saveAlertConfig = async () => {
    ...
    try {
      ...
      const response = await publicApi.put("/api/inventory/alerts-config", payload, { headers: { Authorization: `Bearer ${token}` } });
      ...
      setAlertConfig({
        ...
        ceoPhone: savedConfig.ceoPhone || "", adminPhone: savedConfig.adminPhone || "",
      });
    ...
    } finally { setIsAlertConfigSaving(false); }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await publicApi.delete(`/api/inventory/${itemToDelete._id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const syncAllProducts = async () => {
    try {
      setIsSyncing(true);
      const token = localStorage.getItem("token");
      await publicApi.post("/api/inventory/sync", {}, { headers: { Authorization: `Bearer ${token}` } });
      showSuccessModal("Sync initiated successfully!");
    } catch { showErrorModal("Failed to initiate sync."); }
    finally { await fetchInventoryData(); setIsSyncing(false); }
  };

  const exportToCSV = () => {
    const headers = ["retailer_id","name","color","size","variant_group","description","availability","url","price","image_url","additional_images","inventory","condition"];
    const rows = filteredInventory.map(item =>
      headers.map(key => {
        const rawValue = key === "additional_images" ? (item.additional_images || []).join(", ") : (item[key] ?? "");
    ...
    return inventoryData
      ...
      .filter(item => {
        ...
        return (
          ...
          item.variant_group?.toLowerCase().includes(term) ||
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="min-w-full table-fixed text-sm border-collapse">
          ...
          <tbody>
            {isLoading ? (
              <tr><td colSpan={7} className="py-8 text-center text-gray-500">Loading inventory…</td></tr>
            ) : filteredInventory.length === 0 ? (
              <tr><td colSpan={7} className="py-8 text-center text-gray-500">No products found.</td></tr>
            ) : (
              filteredInventory.map((item, index) => (
                <tr key={item._id} className={`border-b border-gray-100 ${index % 2 === 0 ? "bg-green-50/30" : "bg-white"} hover:bg-green-50 transition`}>
                  ...
                  <td className="py-3 px-4 align-middle">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-md bg-gray-100 ring-1 ring-gray-200 overflow-hidden flex-shrink-0 relative">
                        <img
                          src={item.image_url?.startsWith("http") ? item.image_url : `${process.env.REACT_APP_BACKEND_URL || ""}/${item.image_url}`}
                          alt={item.name}
                          className="h-full w-full object-cover"
                  ...
                  <td className="py-3 px-4 text-center align-middle">
                    <Badge tone={item.synced ? "green" : "yellow"} className="px-2 py-0.5 text-[11px]">
                      {item.synced ? <span className="flex items-center justify-center gap-1"><CheckCircle className="h-3 w-3" /> Synced</span> : "Pending"}
                    </Badge>
                  </td>
                  <td className="py-3 px-4 text-center align-middle">
                    <div className="flex justify-center gap-2">
                      <button onClick={() => openEditModal(item)} className="p-2 rounded-md bg-blue-50 text-blue-600 hover:bg-blue-100 transition" title="Edit">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => openDeleteModal(item)} className="p-2 rounded-md bg-red-50 text-red-600 hover:bg-red-100 transition" title="Delete">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
    ...
    <div className="min-h-full px-3 sm:px-4 md:px-6 lg:px-1 pb-8 sm:pb-10 sm:pt-5">
          ...
          <div className="flex items-start justify-between gap-3 border-b border-gray-100 pb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Send WhatsApp alert when stock drops below threshold.</p>
              </div>
            </div>
            <button type="button" onClick={() => setIsAlertModalOpen(false)} className="rounded-full p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-700">
              <X className="h-4 w-4" />
            </button>
          </div>
      ...
      <div className="mb-3 space-y-2.5 sm:space-y-0 sm:flex sm:items-center sm:justify-between sm:gap-3">
        <div className="flex items-center gap-2 sm:flex-1 sm:gap-3">
          <div className="relative flex-1 min-w-0 sm:min-w-[260px]">
            ...
            <input type="text" placeholder="Search products, IDs…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
            ...
            {searchTerm && (
              <button type="button" onClick={() => setSearchTerm("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 rounded-full p-0.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition">
        ...
        <div className="scrollbar-hide flex items-center gap-2 overflow-x-auto sm:overflow-visible sm:gap-2.5 -mx-3 px-3 sm:mx-0 sm:px-0 pb-0.5 sm:pb-0">
          <button onClick={openAddModal} className="flex-shrink-0 inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg bg-green-600 px-3 py-2 text-xs sm:text-sm font-semibold text-white shadow-sm hover:bg-green-700 active:scale-[.97] transition">
            <Plus className="h-4 w-4" /><span className="hidden min-[400px]:inline">Add</span>
          </button>
          <button onClick={() => setIsAlertModalOpen(true)} className="flex-shrink-0 inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg border border-green-400 bg-white px-3 py-2 text-xs sm:text-sm text-green-700 shadow-sm hover:bg-green-50 active:scale-[.97] transition">
            <BellRing className="h-4 w-4" /><span className="hidden min-[400px]:inline sm:hidden lg:inline">Alert</span>
          </button>
          <div className="flex-shrink-0 inline-flex items-center rounded-lg border border-green-400 bg-white p-[3px] shadow-sm">
            <button onClick={() => setView("list")} className={`rounded-md p-[7px] transition ${view === "list" ? "bg-green-600 text-white shadow" : "text-gray-500 hover:bg-green-50"}`} title="List view">
              <List className="h-4 w-4" />
            </button>
            <button onClick={() => setView("grid")} className={`rounded-md p-[7px] transition ${view === "grid" ? "bg-green-600 text-white shadow" : "text-gray-500 hover:bg-green-50"}`} title="Grid view">
              <LayoutGrid className="h-4 w-4" />
            </button>
          </div>
          <button onClick={exportToCSV} className="flex-shrink-0 inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg border border-green-400 bg-white px-3 py-2 text-xs sm:text-sm text-green-700 shadow-sm hover:bg-green-50 active:scale-[.97] transition">
            <Download className="h-4 w-4" /><span className="hidden min-[400px]:inline">Export</span>
          </button>
          <button onClick={syncAllProducts} disabled={isSyncing} className={`flex-shrink-0 inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg px-3 py-2 text-xs sm:text-sm font-semibold text-white shadow-sm active:scale-[.97] transition ${isSyncing ? "bg-green-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"}`}>
            <RefreshCw className={`h-4 w-4 ${isSyncing ? "animate-spin" : ""}`} />
            <span className="hidden min-[400px]:inline">{isSyncing ? "Syncing…" : "Sync"}</span>
          </button>
        </div>
      </div>
      ...
      <div className="mb-3 flex items-center justify-between px-0.5">
        <p className="text-[11px] sm:text-xs text-gray-500">
          Showing <span className="font-semibold text-gray-800">{filteredInventory.length}</span>
          {filteredInventory.length !== inventoryData.length && <span className="text-gray-400"> of {inventoryData.length}</span>}{" "}
          product{filteredInventory.length !== 1 ? "s" : ""}
          {filterStatus !== "all" && <Badge tone={filterStatus === "synced" ? "green" : "yellow"} className="ml-2">{filterStatus}</Badge>}
        </p>
      </div>
            ...
            {isLoading
              ...
              ? (
                <div className="col-span-full flex flex-col items-center justify-center text-center py-12 sm:py-16">
                  ...
                  <h3 className="mt-3 sm:mt-4 text-base sm:text-lg font-semibold text-gray-900">No products found</h3>
                  <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-500 px-4">Try a different search or filter.</p>
                </div>
              )
