...
import {
  ...
  Zap, Globe, ChevronRight, AlertCircle, ShoppingBag,
  Settings, Package, ArrowRight, ExternalLink, Shield
} from 'lucide-react';
import { toast, ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';

/* ─────────────────────────────────────────────
   Tiny utility: styled text input / textarea
───────────────────────────────────────────── */
...
const Toggle = ({ checked, onChange }) => (
  <button
    ...
    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:ring-offset-2
...
───────────────────────────────────────────── */
const SectionHeader = ({ icon: Icon, title, subtitle, enabled, onToggle, color = 'emerald' }) => (
  <div className={`flex items-center justify-between p-5 rounded-2xl border
    ${enabled ? `bg-${color}-50/60 border-${color}-100` : 'bg-slate-50 border-slate-200'}`}>
    <div className="flex items-center gap-3.5">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0
        ${enabled ? `bg-${color}-100 text-${color}-600` : 'bg-slate-100 text-slate-400'}`}>
...
/* ─────────────────────────────────────────────
   Info banner
───────────────────────────────────────────── */
const InfoBanner = ({ children, color = 'blue' }) => (
  <div className={`flex gap-3 p-4 rounded-xl bg-${color}-50 border border-${color}-100`}>
    ...
    <p className={`text-xs text-${color}-700 leading-relaxed`}>{children}</p>
  </div>
);
...
const OrderAutomationConfig = () => {
  ...
  const [config, setConfig] = useState({
    automationConfig: {
      ...
      paymentRequest: {
        ...
        stripeConfig: { currency: 'inr', successUrl: '', cancelUrl: '' },
        cashfreeConfig: {
          clientId: '',
          clientSecret: '',
          environment: 'sandbox',
  ...
  const fetchAllSettings = async () => {
    try {
      ...
      const [settingsRes, counterRes] = await Promise.all([
        axios.get('/api/catalog-settings', { headers }),
        axios.get('/api/orders/counter/current', { headers })
      ]);

      if (settingsRes.data.settings) {
        const db = settingsRes.data.settings;
        setConfig(prev => ({
          ...prev, ...db,
          automationConfig: {
            ...
            paymentRequest: {
              ...
              stripeConfig: { ...prev.automationConfig.paymentRequest.stripeConfig, ...db.automationConfig?.paymentRequest?.stripeConfig },
              cashfreeConfig: { ...prev.automationConfig.paymentRequest.cashfreeConfig, ...db.automationConfig?.paymentRequest?.cashfreeConfig },
              template: { ...prev.automationConfig.paymentRequest.template, ...db.automationConfig?.paymentRequest?.template }
            },
            abandonedCart: {
            ...
            orderConfirmation: { ...prev.automationConfig.orderConfirmation, ...db.automationConfig?.orderConfirmation },
            shippingUpdate: { ...prev.automationConfig.shippingUpdate, ...db.automationConfig?.shippingUpdate },
            orderIdConfig: { ...prev.automationConfig.orderIdConfig, ...db.automationConfig?.orderIdConfig }
          }
        }));
      }
      if (counterRes.data.nextOrderId) setOrderCounter(counterRes.data.nextOrderId);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    const headers = { Authorization: `Bearer ${token}` };
    try {
      await axios.post('/api/catalog-settings', { ...config, orderCounter, orderIdPrefix: config.automationConfig.orderIdConfig?.prefix }, { headers });
      await axios.put('/api/orders/counter/set', { counterValue: orderCounter }, { headers });
    ...
    <div className="min-h-screen bg-slate-50/70 font-sans">
      ...
      <div className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-slate-100 px-6 py-3.5 flex items-center justify-between">
        ...
        <button onClick={handleSave} disabled={saving}
          className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold rounded-xl transition-colors duration-150 disabled:opacity-60 shadow-sm shadow-emerald-200">
          ...
          <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
              ...
              <>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  <Field label="Meta Flow ID" hint="The Flow ID from your Meta Business Manager">
                    <Input value={ac.orderFlow.flowId} onChange={e => updateConfig('orderFlow', 'flowId', e.target.value)} placeholder="e.g. 123456789012345" />
                  </Field>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  ...
                  <Field label="Header">
                    <Input value={ac.orderFlow.template.header} onChange={e => updateTemplate('orderFlow', 'header', e.target.value)} placeholder="🛍️ Complete Your Order" />
                  </Field>
                  <Field label="Body">
                    <Textarea rows={4} value={ac.orderFlow.template.body} onChange={e => updateTemplate('orderFlow', 'body', e.target.value)} placeholder="Please review your items and confirm your order." />
                  </Field>
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Footer">
                      <Input value={ac.orderFlow.template.footer} onChange={e => updateTemplate('orderFlow', 'footer', e.target.value)} placeholder="Powered by GoWhats!" />
                    </Field>
                    <Field label="Button Label">
                      <Input value={ac.orderFlow.template.ctaText} onChange={e => updateTemplate('orderFlow', 'ctaText', e.target.value)} placeholder="Checkout Now" />
                    </Field>
                  </div>
                </div>
              </>
            ...
            {activeTab === 'shipping' && (
              <>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      { val: 'customer_choice', title: 'Customer selects', desc: 'A list of courier options is sent to the customer', badge: 'New' }
                    ].map(opt => (
                ...
                {/* Courier list template */}
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  <div className="flex items-center gap-2">
                    <Truck className="w-4 h-4 text-emerald-600" />
                    <h3 className="text-sm font-semibold text-slate-700">Courier Selection Message</h3>
                  </div>
                  <Field label="Header">
                    <Input value={ac.shippingSelection.courierListTemplate.header} onChange={e => updateNested('shippingSelection', 'courierListTemplate', 'header', e.target.value)} />
                  </Field>
                  <Field label="Body" hint="Variables: {{name}}, {{amount}}">
                    <Textarea rows={3} value={ac.shippingSelection.courierListTemplate.body} onChange={e => updateNested('shippingSelection', 'courierListTemplate', 'body', e.target.value)} />
                  </Field>
                  <Field label="Footer">
                    <Input value={ac.shippingSelection.courierListTemplate.footer} onChange={e => updateNested('shippingSelection', 'courierListTemplate', 'footer', e.target.value)} />
                  </Field>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-slate-700">Free Shipping Message</h3>
                  </div>
                  <Field label="Body" hint="Variable: {{method}}">
                    <Textarea rows={3} value={ac.shippingSelection.freeShippingTemplate.body} onChange={e => updateNested('shippingSelection', 'freeShippingTemplate', 'body', e.target.value)} />
                  </Field>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-slate-700">Tracking Update</h3>
                  </div>
                  <Field label="Message Body" hint="Variables: {{order_id}}, {{courier}}, {{tracking_no}}">
                    <Textarea rows={4} value={ac.shippingUpdate.template.body} onChange={e => updateTemplate('shippingUpdate', 'body', e.target.value)} />
                  </Field>
                </div>
              </>
            )}
            ...
            {activeTab === 'payment' && (
              <>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  <div>
                    <h4 className="text-sm font-semibold text-slate-800">Payment Gateway</h4>
                    <p className="text-xs text-slate-400 mt-0.5">Choose the gateway to collect payments from customers</p>
                  </div>
                  <div className="space-y-2.5">
                    <GatewayCard id="razorpay" selected={gw} onChange={v => updateConfig('paymentRequest', 'paymentGateway', v)}
                    ...
                    <GatewayCard id="stripe" selected={gw} onChange={v => updateConfig('paymentRequest', 'paymentGateway', v)}
                    ...
                    <GatewayCard id="cashfree" selected={gw} onChange={v => updateConfig('paymentRequest', 'paymentGateway', v)}
                      icon={Shield} name="Cashfree"
                      tagline="UPI payment via Cashfree — creates order, sends UPI intent link via WhatsApp"
                ...
                {gw === 'razorpay' && (
                  <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                    <h4 className="text-sm font-semibold text-slate-800">Razorpay Configuration</h4>
                    <Field label="Configuration Name" hint="The name set up in WhatsApp Business Manager for Razorpay">
                    ...
                    <InfoBanner color="blue">
                      Razorpay UPI sends a native WhatsApp Pay bubble. The customer pays inside WhatsApp.
                      Payment webhook arrives at <code className="bg-blue-100 px-1 rounded font-mono">/webhook/razorpay/partner</code>.
                    </InfoBanner>
                  </div>
                )}
                ...
                {gw === 'stripe' && (
                  <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                    <div className="flex items-start justify-between">
                      <h4 className="text-sm font-semibold text-slate-800">Stripe Configuration</h4>
                      <span className="flex items-center gap-1 text-xs text-slate-400 bg-slate-100 px-2 py-1 rounded-lg">
                        <Shield className="w-3 h-3" /> Keys stored server-side
                      </span>
                    </div>
                    ...
                    <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 flex gap-3">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-amber-700 mb-1">API Keys not configured here</p>
                        <p className="text-xs text-amber-600 leading-relaxed">
                          Stripe secret keys are stored as environment variables on the server (<code className="bg-amber-100 px-1 rounded font-mono">STRIPE_SECRET_KEY</code>).
                          Configure them in your server's <code className="bg-amber-100 px-1 rounded font-mono">.env</code> file or deployment environment settings.
                          Only UI/UX preferences are saved here.
                        </p>
                      </div>
                    </div>
                    ...
                    <Field label="Currency">
                      <select className="w-full px-3.5 py-2.5 text-sm bg-white border border-slate-200 rounded-xl text-slate-800
                        ...
                        onChange={e => updateNested('paymentRequest', 'stripeConfig', 'currency', e.target.value)}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <Field label="Success URL" hint="Optional redirect after payment">
                        <Input value={ac.paymentRequest.stripeConfig.successUrl}
                          onChange={e => updateNested('paymentRequest', 'stripeConfig', 'successUrl', e.target.value)}
                          placeholder="https://yoursite.com/thank-you" />
                      </Field>
                      <Field label="Cancel URL" hint="Optional redirect on cancellation">
                        <Input value={ac.paymentRequest.stripeConfig.cancelUrl}
                          onChange={e => updateNested('paymentRequest', 'stripeConfig', 'cancelUrl', e.target.value)}
                          placeholder="https://yoursite.com/cart" />
                      </Field>
                    </div>

                    <InfoBanner color="purple">
                      Stripe sends a <strong>Pay Now</strong> button to the customer. Payment confirmation arrives at&nbsp;
                      <code className="bg-purple-100 px-1 rounded font-mono">/webhook/stripe</code>.
                      Make sure <code className="bg-purple-100 px-1 rounded font-mono">STRIPE_SECRET_KEY</code> and&nbsp;
                      <code className="bg-purple-100 px-1 rounded font-mono">STRIPE_WEBHOOK_SECRET</code> are set in your environment.
                    </InfoBanner>
                  </div>
                )}
                ...
                {gw === 'cashfree' && (
                  <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                    <h4 className="text-sm font-semibold text-slate-800">Cashfree Configuration</h4>

                    {/* Environment */}
                    <div className="grid grid-cols-2 gap-3">
                      {['sandbox', 'production'].map(env => (
                        <label key={env} className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-sm
                          ${ac.paymentRequest.cashfreeConfig.environment === env
                            ? 'border-emerald-500 bg-emerald-50/60 text-emerald-800 font-semibold'
                            : 'border-slate-200 text-slate-600 hover:border-slate-300'}`}>
                          <input type="radio" name="cfEnv" value={env}
                            checked={ac.paymentRequest.cashfreeConfig.environment === env}
                            onChange={() => updateNested('paymentRequest', 'cashfreeConfig', 'environment', env)}
                            className="accent-emerald-600" />
                          <span className="capitalize">{env}</span>
                          {env === 'sandbox' && <span className="ml-auto text-[10px] bg-amber-100 text-amber-600 font-bold px-1.5 py-0.5 rounded-md">TEST</span>}
                          {env === 'production' && <span className="ml-auto text-[10px] bg-emerald-100 text-emerald-700 font-bold px-1.5 py-0.5 rounded-md">LIVE</span>}
                        </label>
                      ))}
                    </div>
                    ...
                    <div className="bg-red-50 border border-red-100 rounded-xl p-4 flex gap-3">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-red-700 mb-1">API Credentials — store securely</p>
                        <p className="text-xs text-red-600 leading-relaxed">
                          Client ID and Secret are saved encrypted. They are used server-side only and never exposed to the browser.
                          Get them from your <a href="https://merchant.cashfree.com" target="_blank" rel="noopener noreferrer" className="underline font-semibold">Cashfree Dashboard <ExternalLink className="w-2.5 h-2.5 inline" /></a>.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <Field label="Client ID">
                        <Input type="password"
                          ...
                          onChange={e => updateNested('paymentRequest', 'cashfreeConfig', 'clientId', e.target.value)}
                          placeholder="••••••••••••••••" />
                      </Field>
                      <Field label="Client Secret">
                        <Input type="password"
                          value={ac.paymentRequest.cashfreeConfig.clientSecret}
                          onChange={e => updateNested('paymentRequest', 'cashfreeConfig', 'clientSecret', e.target.value)}
                          placeholder="••••••••••••••••" />
                      </Field>
                    </div>

                    <Field label="Merchant VPA" hint="The UPI VPA registered with Cashfree (e.g. merchant@yesbank)">
                      <Input value={ac.paymentRequest.cashfreeConfig.merchantVpa}
                        onChange={e => updateNested('paymentRequest', 'cashfreeConfig', 'merchantVpa', e.target.value)}
                        placeholder="merchant@yesbank" />
                    </Field>

                    <Field label="WhatsApp Payment Configuration Name" hint="The configuration name in WhatsApp Business Manager linked to this VPA">
                      <Input value={ac.paymentRequest.cashfreeConfig.paymentConfigurationName}
                        onChange={e => updateNested('paymentRequest', 'cashfreeConfig', 'paymentConfigurationName', e.target.value)}
                        placeholder="e.g. cashfree_upi_config" />
                    </Field>

                    <Field label="Notify URL (Webhook)" hint="Cashfree will POST payment status to this URL">
                      <Input value={ac.paymentRequest.cashfreeConfig.notifyUrl}
                        onChange={e => updateNested('paymentRequest', 'cashfreeConfig', 'notifyUrl', e.target.value)}
                        placeholder="https://yourdomain.com/webhook/cashfree" />
                    </Field>
                    ...
                    <Field label="Currency">
                      <select className="w-full px-3.5 py-2.5 text-sm bg-white border border-slate-200 rounded-xl text-slate-800
                        ...
                        onChange={e => updateNested('paymentRequest', 'cashfreeConfig', 'currency', e.target.value)}>
                        <option value="INR">INR — Indian Rupee (₹)</option>
                      </select>
                    </Field>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2">
                      <p className="text-xs font-semibold text-slate-600 mb-3">How Cashfree UPI works</p>
                      {[
                        ['1', 'Customer places order on WhatsApp'],
                        ['2', 'Server creates Cashfree order → gets UPI session ID'],
                        ['3', 'UPI intent URL extracted → sent to customer via WhatsApp Pay'],
                        ['4', 'Customer pays in GPay / PhonePe / Paytm'],
                        ['5', 'Cashfree webhook confirms payment → order confirmed'],
                      ].map(([n, t]) => (
                        <div key={n} className="flex items-center gap-2.5">
                          <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-bold flex items-center justify-center flex-shrink-0">{n}</span>
                    ...
                    <InfoBanner color="amber">
                      Payment confirmation webhook must come from Cashfree, not WhatsApp — always rely on the Cashfree webhook for final payment status.
                      Webhook endpoint: <code className="bg-amber-100 px-1 rounded font-mono">/webhook/cashfree</code>.
                    </InfoBanner>
                  </div>
                )}
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  ...
                  <Field label="Header">
                    <Input maxLength={60} placeholder="💳 Complete Your Payment"
                      value={ac.paymentRequest.template.header}
                      onChange={e => updateTemplate('paymentRequest', 'header', e.target.value)} />
                  </Field>
                  <Field label="Body">
                    <Textarea rows={3} placeholder="Please complete your payment of {{amount}} for order {{order_id}}."
                      value={ac.paymentRequest.template.body}
                      onChange={e => updateTemplate('paymentRequest', 'body', e.target.value)} />
                  ...
                  <Field label="Footer">
                    <Input maxLength={60} placeholder="Secure Payment"
                      value={ac.paymentRequest.template.footer}
                      onChange={e => updateTemplate('paymentRequest', 'footer', e.target.value)} />
                  </Field>
                </div>
              </>
            )}
              ...
              <>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  <Field label="Message Body">
                    <Textarea rows={6} value={ac.orderConfirmation.template.body} onChange={e => updateTemplate('orderConfirmation', 'body', e.target.value)}
              ...
              <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Delay (Minutes)" hint="Recommended: 30–60 min">
                      <Input type="number" min="5" max="1440"
                        value={ac.abandonedCart.delayMinutes}
                        onChange={e => updateConfig('abandonedCart', 'delayMinutes', parseInt(e.target.value))} />
                    </Field>
                    <Field label="Button URL" hint="Link opened when customer taps the CTA">
                      <Input value={ac.abandonedCart.buttonLink || ''}
                        onChange={e => updateConfig('abandonedCart', 'buttonLink', e.target.value)}
                        placeholder="https://wa.me/c/91XXXXXXXXXX" />
                    </Field>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  ...
                  <div className="bg-[#ece5dd] rounded-2xl p-4 max-w-xs mx-auto">
                    <div className="bg-white rounded-xl rounded-tl-sm p-3 shadow-sm space-y-1.5 text-sm">
                      {ac.abandonedCart.template.header && <p className="font-bold text-slate-800">{ac.abandonedCart.template.header}</p>}
            ...
            {activeTab === 'order_id' && (
              <>
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Order Prefix" hint='Short code like "ORD", "EV", "SHOP"'>
                      <Input maxLength={5}
                        className="uppercase font-mono font-bold text-base tracking-widest"
                    ...
                    <Field label="Next Order Number">
                      <Input type="number" min="1"
                        ...
                        value={orderCounter}
                        onChange={e => setOrderCounter(e.target.value)} />
                    </Field>
                  </div>
