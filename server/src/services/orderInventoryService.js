...
const whatsappSync = require('./whatsappSync');
...
async function sendLowStockAlerts({
  ...
  for (const recipient of recipients) {
    ...
    let sent = false;
    const templateNameRaw = asString(config.templateName);
    const templateNameNormalized = templateNameRaw.toLowerCase().replace(/\s+/g, '_');
    const templateCandidates = Array.from(
      new Set([templateNameRaw, templateNameNormalized].filter(Boolean))
    );
    const languageCandidates = Array.from(
      new Set([asString(config.templateLanguage) || 'en', 'en'])
    );
      ...
      for (const candidateTemplate of templateCandidates) {
        ...
        for (const candidateLanguage of languageCandidates) {
          ...
          try {
            ...
            console.warn(
              `Low stock template send failed for ${formattedPhone} (template=${candidateTemplate}, lang=${candidateLanguage}):`,
...
async function applyInventoryDeductionForOrder(order) {
  try {
    ...
    for (const item of items) {
      ...
      // Re-sync immediately so UI keeps "Synced" after inventory auto-decrease.
      let latestItem = updatedItem;
      try {
        await whatsappSync.syncSingleProduct(updatedItem);
        const refreshedItem = await InventoryItem.findById(updatedItem._id);
        if (refreshedItem) {
          latestItem = refreshedItem;
        }
      } catch (syncError) {
        console.error(
          `Inventory auto-sync failed for ${updatedItem._id}:`,
          syncError?.message || syncError
        );
      }

      if (updatedStock > alertConfig.threshold && latestItem.low_stock_alert_sent) {
        await InventoryItem.updateOne(
          { _id: latestItem._id, tenant_id: tenantId },
          {
            $set: {
      ...
      if (
        isLowStock &&
        !latestItem.low_stock_alert_sent &&
        shouldAttemptAlerts &&
        whatsappService
      ) {
          ...
          await InventoryItem.updateOne(
            { _id: latestItem._id, tenant_id: tenantId },
            {
              $set: {
                ...
                low_stock_alert_sent_at: new Date(),
                low_stock_alert_recipients: sentRecipients,
                low_stock_alert_threshold: alertConfig.threshold
              }
            }
          );
        }
